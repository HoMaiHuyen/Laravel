...
@endif
    ...
    <form action="{{ route('users.post-edit', ['id' => $userDetail->id]) }}" method="POST">
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            <select name="status" class="form-control" id="">
                <option value="0" {{old('status')==0 || $userDetail->status==0? 'selected':false}}>Not activated</option>
                <option value="1" {{old('status')==1 || $userDetail->status==1? 'selected':false}}>Activated</option>
            </select>
        </div>
